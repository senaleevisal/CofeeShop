...
public class Main {
    ...
    private static void PlaseOder () {
        //This methos is used to make new oder.... but first first of all check the customer is already exists or not
        //Weather it is not registerd we make a new customer and place order
        L3:while(true) {
            ...
            System.out.println("\t\n\nOrder ID : ODR#" + oderIdWithFiveDigit ); // make a new ID for the oder...
            System.out.println();
    ...
    private static void addEmployee() { //<------This method is used Add the employer for this System.
        new UI().addEmployeeUi();
        L2:while(true) {
            int emplyeeId = Array_Handler.employeerId;
            System.out.print("\nEnter Employee nic\t: ");
            String employeerNic = input.next();
            if (cofeeShop.getEmployees().available_object(employeerNic)) { //<-----check this employer is already register or not register....
                Employee employee = (Employee) cofeeShop.getEmployees().getObject(employeerNic);
                System.out.println(employee.getName() + " is already registers...!");
                System.out.println("Employer Id is\t:" + String.format("%05d", employee.getId()) );
            } else {
                System.out.println("Do you want to add Employee..? (Y/N)");
                String YesOrNo1 = input.next();
                if (YesOrNo1.equalsIgnoreCase("yes") || YesOrNo1.equalsIgnoreCase("y")) {
                    System.out.println("Employee ID is\t:" + emplyeeId+1);
                    System.out.print("Enter Employee Name\t:");
                    String employeerName = input.next();
                    System.out.print("Enter your email\t:");
                    String employeerEmail = input.next();
                    System.out.print("Enter your role\t:");
                    String employeerrole = input.next();
                    System.out.println("Do you want to add this Employee..? (Y/N)");
                    String YesOrNo2 = input.next();
                    if (YesOrNo2.equalsIgnoreCase("yes") || YesOrNo2.equalsIgnoreCase("y")) {
                        int employeerId = Array_Handler.employeerId+1; // <---- Make employer ID...
                        Employee employee = new Employee(employeerId,employeerName,employeerrole, employeerNic,employeerEmail);
                        Array_Handler.employeerId = employeerId;
                        cofeeShop.addEmployee(employee);
                    }else {
    ...
    private static void SearchOder() {
        System.out.print("Enter order id : ");
        int orderId = input.nextInt();
        if(cofeeShop.getOrders().available_object(orderId)){
            Order order = (Order)cofeeShop.getOrders().getObject(orderId);
            System.out.printf("\t\t+---------------+-------------+-----------+\n");
            System.out.printf("\t\t|   %-11s | %-11s | %-9s |  %-9s | %-12s |\n","Item ID", "Customer Name","Employee name ","price");
            System.out.printf("\t\t+---------------+-------------+-----------+------------+\n");
            System.out.printf("\t\t|   %-11s | %-11s | %-9s |  %-9s | %-12s |\n\n",order.getId(), order.getCustomer().getName(),order.getEmployee().getName(),order.getPrice());
        }
    }

    private static void deleteOder() {
        ...
        String YesOrNo1 = input.next();
        if (YesOrNo1.equalsIgnoreCase("yes") || YesOrNo1.equalsIgnoreCase("y")) {
            cofeeShop.deleteOrder(orderId);
        }else {
            System.out.println("ok then order keep as it is");
        }
    }
